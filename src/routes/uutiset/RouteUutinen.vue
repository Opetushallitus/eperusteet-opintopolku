...
const props = defineProps({
  ...
  julkaistutKoulutustyypitStore: {
    type: Object as () => JulkaistutKoulutustyypitStore,
    required: true,
  },
});
...
onMounted(() => {
  if (route) {
    props.tiedoteStore.fetchUutinen(_.parseInt(route.params.tiedoteId as string));
  }
});

const tiedote = computed(() => {
  return props.tiedoteStore.tiedote;
});
...
const perusteet = computed(() => {
  return _.filter(props.tiedoteStore.tiedote?.perusteet, peruste =>
...
const osaamisalojenPerusteet = computed(() => {
  return props.tiedoteStore.tiedotteenOsaamisalaPerusteet;
});

const tutkinnonosienPerusteet = computed(() => {
  return props.tiedoteStore.tiedotteenTutkinnonosaPerusteet;
});
