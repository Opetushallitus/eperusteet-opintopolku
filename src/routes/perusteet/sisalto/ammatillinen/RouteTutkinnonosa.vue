    ...
    router.replace(
      {
        name: 'tutkinnonosa',
        params: {
          perusteId: _.toString(props.perusteDataStore.peruste?.id),
...
const tutkinnonosaViite = computed(() => {
  return props.perusteDataStore.getJulkaistuPerusteSisalto({ id: tutkinnonosaViiteId.value }) as any;
});

const perusteenTutkinnonosaViitteet = computed(() => {
  return _.chain(props.perusteDataStore.getJulkaistuPerusteSisalto('suoritustavat'))
...
const perusteenTutkinnonosa = computed(() => {
  return props.perusteDataStore.getJulkaistuPerusteSisalto({ id: _.toNumber(_.get(tutkinnonosaViite.value, '_tutkinnonOsa')) }) as any;
});
...
const arviointiasteikot = computed(() => {
  return props.perusteDataStore.arviointiasteikot;
});
...
const perusteenKielet = computed(() => {
  return props.perusteDataStore.peruste?.kielet;
});
