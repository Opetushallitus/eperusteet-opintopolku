...
const peruste = computed(() => {
  return props.perusteDataStore.peruste;
});

const rakenne = computed((): any => {
  return _.get(_.first(props.perusteDataStore.getJulkaistuPerusteSisalto('suoritustavat')), 'rakenne');
});

const perusteenTutkinnonosaViitteetById = computed(() => {
  return _.keyBy(_.chain(props.perusteDataStore.getJulkaistuPerusteSisalto('suoritustavat'))
...
const perusteenTutkinnonosatById = computed(() => {
  return _.keyBy(props.perusteDataStore.getJulkaistuPerusteSisalto('tutkinnonOsat'), 'id');
});
