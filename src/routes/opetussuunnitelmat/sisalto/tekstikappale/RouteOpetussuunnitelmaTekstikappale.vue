...
const tekstiKappaleViite = computed(() => {
  return props.opetussuunnitelmaDataStore.getJulkaistuSisalto({ id: tekstikappaleId.value });
});
...
const perusteTekstikappaleViite = computed(() => {
  return props.opetussuunnitelmaDataStore.getJulkaistuPerusteSisalto({ _perusteenOsa: _.toString(tekstiKappaleViite.value.perusteTekstikappaleId) });
});
...
const current = computed(() => {
  return props.opetussuunnitelmaDataStore.current || null;
});

const pohjaNimi = computed(() => {
  return props.opetussuunnitelmaDataStore.opetussuunnitelma?.pohja?.nimi;
});

const laajaAlaisetOsaamiset = computed(() => {
  if (perusteTekstikappale.value?.tunniste === 'laajaalainenosaaminen') {
    return props.opetussuunnitelmaDataStore.getJulkaistuPerusteSisalto('aipe.laajaalaisetosaamiset');
  }
  return undefined;
});
