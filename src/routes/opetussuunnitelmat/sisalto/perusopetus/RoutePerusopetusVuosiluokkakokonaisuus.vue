...
const vuosiluokkakokonaisuus = computed(() => {
  return props.opetussuunnitelmaDataStore.getJulkaistuSisalto({ id: vlkId.value });
});

const pohjanVuosiluokkakokonaisuus = computed(() => {
  const opsVlk = _.find(props.opetussuunnitelmaDataStore.getJulkaistuSisalto('vuosiluokkakokonaisuudet'),
...
const perusteenVuosiluokkakokonaisuus = computed(() => {
  return props.opetussuunnitelmaDataStore.getJulkaistuPerusteSisalto({ tunniste: vuosiluokkakokonaisuus.value._tunniste });
});
...
const laajaalaisetOsaamiset = computed(() => {
  return _.chain(props.opetussuunnitelmaDataStore.getJulkaistuPerusteSisalto('perusopetus.laajaalaisetosaamiset'))
    .map(lao => {
      return {
        ...lao,
