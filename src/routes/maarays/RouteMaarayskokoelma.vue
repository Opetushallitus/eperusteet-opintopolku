...
const maarayksetStore = ref<MaarayksetStore | null>(null);
...
onMounted(async () => {
  ...
  maarayksetStore.value = new MaarayksetStore();
  await maarayksetStore.value.init();
  await fetch();
});
...
async function fetch() {
  if (_.size(query.value.nimi) === 0 || _.size(query.value.nimi) > 2) {
    await maarayksetStore.value?.fetch(
      {
        ...query.value,
...
const maaraykset = computed(() => {
  return maarayksetStore.value?.maaraykset?.value?.data;
});

const maarayksetCount = computed(() => {
  return maarayksetStore.value?.maaraykset?.value?.kokonaismäärä;
});
...
const koulutustyyppiVaihtoehdot = computed(() => {
  return maarayksetStore.value?.koulutustyypit.value;
});
