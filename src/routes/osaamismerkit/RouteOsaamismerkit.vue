...
const osaamismerkit = computed(() => {
  return osaamismerkitStore.osaamismerkit.value;
});

const osaamismerkkiKategoriat = computed(() => {
  return osaamismerkitStore.kategoriat.value;
});

watch(query, async (newQuery: OsaamismerkitQuery) => {
  if (_.size(newQuery.nimi) === 0 || _.size(newQuery.nimi) > 2) {
    await osaamismerkitStore.updateOsaamismerkkiQuery({
      ...newQuery,
    });
  }
...
const osaamismerkkiKategoriaOptionsMapped = computed(() => {
  if (!osaamismerkitStore.kategoriat.value) {
    return null;
  }

  return _.chain(osaamismerkitStore.kategoriat.value)
    .map(kategoria => {
      return {
