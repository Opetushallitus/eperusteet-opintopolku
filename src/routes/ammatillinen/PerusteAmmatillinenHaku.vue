...
const props = defineProps({
  perusteHakuStore: {
    type: Object as () => IPerusteHakuStore,
    required: true,
  },
  tyyppi: {
...
const updateFilters = async (filters) => {
  if (_.size(filters.nimiTaiKoodi) === 0 || _.size(filters.nimiTaiKoodi) > 2) {
    await props.perusteHakuStore.updateFilters(filters);
  }
};
...
onMounted(async () => {
  ...
  if (!props.perusteHakuStore.perusteet) {
    await props.perusteHakuStore.fetch();
  }
});
...
const page = computed({
  get: () => {
    return props.perusteHakuStore.page + 1;
  },
  set: (value) => {
    props.perusteHakuStore.page = value - 1;
  },
});

watch(page, async () => {
  await updateFilters({ sivu: props.perusteHakuStore.page });
...
const perusteetJaTutkinnonosat = computed(() => {
  if (props.perusteHakuStore.perusteet) {
    return mapPerusteet(props.perusteHakuStore.perusteet);
  }
  return [];
});

const total = computed(() => {
  return props.perusteHakuStore.total;
});

const pages = computed(() => {
  return props.perusteHakuStore.pages;
});

const perPage = computed(() => {
  return props.perusteHakuStore.perPage;
});

const filters = computed(() => {
  return props.perusteHakuStore.filters;
});
