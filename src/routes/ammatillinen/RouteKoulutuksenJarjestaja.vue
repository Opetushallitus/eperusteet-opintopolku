...
const props = defineProps({
  koulutuksenJarjestajaStore: {
    type: Object as () => KoulutuksenJarjestajaStore,
    required: true,
  },
});
...
const koulutustoimija = computed(() => {
  return props.koulutuksenJarjestajaStore.koulutustoimija.value;
});

const yhteisetOsuudet = computed(() => {
  if (props.koulutuksenJarjestajaStore.yhteisetOsuudet.value) {
    return _.map(props.koulutuksenJarjestajaStore.yhteisetOsuudet.value, yhteinenOsuus => {
      return {
        ...yhteinenOsuus,
...
const toteutussuunnitelmat = computed(() => {
  if (props.koulutuksenJarjestajaStore.toteutussuunnitelmat.value) {
    return _.map(props.koulutuksenJarjestajaStore.toteutussuunnitelmat.value.data, toteutussuunnitelma => {
      return {
        ...toteutussuunnitelma,
        route: {
...
const toteutussuunnitelmaTotal = computed(() => {
  return props.koulutuksenJarjestajaStore.toteutussuunnitelmat.value?.kokonaismäärä;
});
...
watch(kieli, async () => {
  await Promise.all([doFetch(), props.koulutuksenJarjestajaStore.fetch()]);
}, { immediate: true });

const doFetch = async () => {
  await props.koulutuksenJarjestajaStore.fetchToteutussuunnitelmat(opsQuery.value, opsPage.value - 1);
};
