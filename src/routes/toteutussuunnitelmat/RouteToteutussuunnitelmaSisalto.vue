...
const sisaltoviite = computed(() => {
  const julkaistuSisalto = props.opetussuunnitelmaDataStore.getJulkaistuSisalto({ id: sisaltoviiteId.value });

  if (_.get(julkaistuSisalto, 'tosa')) {
    const tutkinnonosat = props.opetussuunnitelmaDataStore.getJulkaistuSisalto('tutkinnonOsat');
...
const perusteenTutkinnonosaViite = computed(() => {
  return _.find(props.opetussuunnitelmaDataStore.perusteidenTutkinnonOsienViitteet, perusteTosaViite =>
    _.get(perusteTosaViite, '_tutkinnonOsa') === _.toString(perusteenTutkinnonOsanId.value));
});

const perusteenTutkinnonosa = computed(() => {
  return _.find(props.opetussuunnitelmaDataStore.perusteidenTutkinnonOsat, perusteTosaViite =>
...
const kuvat = computed(() => {
  return [
    ...props.opetussuunnitelmaDataStore.kuvat,
    ...props.opetussuunnitelmaDataStore.perusteKuvat,
  ];
});

const arviointiasteikot = computed(() => {
  return props.opetussuunnitelmaDataStore.arviointiasteikot;
});

const current = computed((): NavigationNode | null => {
  return props.opetussuunnitelmaDataStore.current;
});
