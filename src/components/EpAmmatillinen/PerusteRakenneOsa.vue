...
const toggleKuvaus = (naytaKuvausArg?: boolean) => {
  if (naytaKuvausArg !== undefined) {
    ...
    nextTick(() => {
      _.forEach(rakenneosa.value, (item: any) => item.toggleKuvaus(naytaKuvaus.value));
    });
  }
  else {
...
const toggleRakenne = (naytaRakenneArg?: boolean) => {
  if (naytaRakenneArg !== undefined) {
    ...
    nextTick(() => {
      _.forEach(rakenneosa.value, (item: any) => item.toggleRakenne(naytaRakenne.value));
    });
  }
  else {
...
const rakenneStyle = computed(() => {
  return 'border-color: ' + rakenneNodecolor(props.rakenneosa, null, null);
});
</script>
