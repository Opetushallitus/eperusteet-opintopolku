...
async function queryImpl(query: string) {
  if (query.length > 2) {
    tulokset.value = null;
    const julkaisu = await props.perusteDataStore.peruste;
    const result: any[] = [];
...
const tuloksetSorted = computed(() => {
  return _.chain(tulokset.value)
    .map(tulos => {
      var numerointi = _.find(props.perusteDataStore.flattenedSidenav, { location: tulos.location })?.meta?.numerointi;
      return {
        ...tulos,
        numerointi,
        nimi: (numerointi || '') + ' '
