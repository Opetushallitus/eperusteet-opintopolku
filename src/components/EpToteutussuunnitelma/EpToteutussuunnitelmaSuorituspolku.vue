...
const julkaistuTutkinnonosaViitteet = computed(() => {
  const tutkinnonosatViite = props.opetussuunnitelmaDataStore.getJulkaistuSisalto({ 'tyyppi': 'tutkinnonosat' });
...
const julkaistutTutkinnonOsat = computed(() => {
  return _.filter(props.opetussuunnitelmaDataStore.getJulkaistuSisalto('tutkinnonOsat'), tosa => tosa.tyyppi === 'tutkinnonosa');
});

const perusteenTutkinnonosaViite = (perusteenTutkinnonosaId) => {
  return _.find(props.opetussuunnitelmaDataStore.perusteidenTutkinnonOsienViitteet, perusteTosaViite => _.get(perusteTosaViite, '_tutkinnonOsa') === _.toString(perusteenTutkinnonosaId));
};

const perusteenTutkinnonosa = (perusteenTutkinnonosaId) => {
  return _.find(props.opetussuunnitelmaDataStore.perusteidenTutkinnonOsat, perusteTosa => perusteTosa.id === perusteenTutkinnonosaId);
};
...
const perusteidenTutkinnonosienViitteetById = computed(() => {
  return _.keyBy(props.opetussuunnitelmaDataStore.perusteidenTutkinnonOsienViitteet, 'id');
});

const perusteidenTutkinnonOsatById = computed(() => {
  return _.keyBy(props.opetussuunnitelmaDataStore.perusteidenTutkinnonOsat, 'id');
});
