...
namespace Controllers {
  export const EsitysController = ($scope, $rootScope, $stateParams, $timeout, sisalto, peruste, YleinenData, $state, Algoritmit,
                                   tutkinnonOsat, Kaanna, arviointiasteikot, koulutusalaService, opintoalaService,
                                   Kieli, TermistoService, MurupolkuData) => {
    ...
    const isTutkinnonosatActive = () => $state.is('root.esitys.peruste.tutkinnonosat') || $state.is('root.esitys.peruste.tutkinnonosa');
    ...
    $scope.arviointiasteikot = _.zipObject(_.map(arviointiasteikot, 'id'), _.map(arviointiasteikot, (asteikko: any) => {
