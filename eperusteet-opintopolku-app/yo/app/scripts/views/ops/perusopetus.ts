...
epOpintopolkuApp
  ...
  .controller('OpsVuosiluokkaController', function(
    ...
    const changeToOppiaine = () => {
      if ($state.is('root.ops.perusopetus.vuosiluokkakokonaisuus.vuosiluokka') && $scope.navi.sections[1].items) {
  ...
  .controller('OpsOppiaineetController', function(
    ...
    $scope.buildVuosiluokatTabit = () => {
      return _.filter($scope.vlkt, function(vlk){
        return !!$scope.vuosiluokkaSisallot[vlk._tunniste]
      });
    };

    $scope.$on('$stateChangeSuccess', () => {
      if (!_.isEmpty($scope.vlktTabit)) {
        var words = _.words($state.current.name);
        if (!_.contains(words, 'vuosiluokat')) {
          $state.go('.vuosiluokat', {vlktId: $scope.vlktTabit[0].id}, {location: 'replace'});
        }
      }
    ...
    const getCurrentVlk = (vlk) => {
      return $state.params.vlktId + "" === vlk.id + "";
    };
    ...
    const getValittuVlk = () => {
      return _.filter(oppiaine.vuosiluokkakokonaisuudet, (vlk) => {
    ...
    $scope.vlktTabit = $scope.buildVuosiluokatTabit();

  })

  .controller('OpsOppiaineetVlktController', function(
    $scope){
  })
  ...
  .controller('OpsVlkOppiaineController', function(
    ...
    $scope.sisalto = opsUtils.makeSisalto(perusteOpVlk, tavoitteet, $scope.perusteOppiaine, laajaalaiset);
    console.log($scope.sisalto)
    $scope.nimiOrder = Utils.sort;
