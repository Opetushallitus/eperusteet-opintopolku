        ...
        $scope.hakuMuuttui = function() {
            ...
            if (nimi && nimi.length > 0) {
                $scope.opsit = _.filter($scope.opsit, function(ops: any) {
                    if (
                        ops.nimi[$scope.kieli] &&
                        _.includes(ops.nimi[$scope.kieli].toLowerCase(), nimi.toLowerCase())
                    ) {
                        return ops;
                    }
                });
            ...
            if ($scope.tarkennettuHaku && avainsana && avainsana.length > 0) {
                $scope.opsit = _.filter($scope.opsit, function(ops: any) {
                    ...
                    _.each(ops.organisaatiot, function(org: any) {
                        if (
                            org.nimi &&
                            org.nimi[$scope.kieli] &&
                            _.includes(org.nimi[$scope.kieli].toLowerCase(), avainsana.toLowerCase())
                        ) {
                            loydetty = true;
                        }
                    });
                    ...
                    _.each(ops.kunnat, function(kunta: any) {
                        if (
                            kunta.nimi &&
                            kunta.nimi[$scope.kieli] &&
                            _.includes(kunta.nimi[$scope.kieli].toLowerCase(), avainsana.toLowerCase())
                        ) {
                            loydetty = true;
                        }
                    });
