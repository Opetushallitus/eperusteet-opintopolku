...
// TODO: Siirrä muualle
angular.module("eperusteet.esitys").directive("epLukioSivunavigaatio", function($compile) {
    return {
...
namespace Controllers {
        ...
        $timeout(function() {
            if ($state.current.name === epEsitysSettings.lukioState) {
                ...
                if (first) {
                    $state.go(
                        ".tekstikappale",
                        { tekstikappaleId: $scope.perusteenSisalto.lapset[0].id },
                        { location: "replace" }
                    );
                }
            }
    ...
    export const epLukioTekstikappaleController = (
        $scope,
        $rootScope,
        $stateParams,
        $location,
        ...
        $rootScope.$on("$locationChangeSuccess", function() {
            ...
            if (lapsi) {
                ...
                MurupolkuData.set(
                    "parents",
                    epParentFinder.find($scope.perusteenSisalto.lapset, tekstikappale.id, true)
                );
    ...
    export const epLukioKurssiController = (
        $scope,
        $stateParams,
        epLukioUtils,
        Kieli,
        ...
        const filterKurssit = id => _.filter(kurssit, (kurssi: any) => "" + kurssi.id === "" + id);
