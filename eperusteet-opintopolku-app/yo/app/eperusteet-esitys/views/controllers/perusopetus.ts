...
namespace Controllers {
    export const epPerusopetusController = (
        ...
        $scope.valitseOppiaineenVuosiluokka = function(vuosiluokka) {
            $timeout(function() {
                ...
                if ($scope.valittuOppiaine.vlks) {
                    $scope.valittuOppiaine.sisallot =
                        $scope.sisallot[$scope.valittuOppiaine.vlks._vuosiluokkaKokonaisuus];
                }
                paivitaTavoitteet();
            });
        };
        ...
        function paivitaTavoitteet(inSisallot = false) {
            if ($scope.valittuOppiaine.vlks) {
                var filteritTyhjat =
                    !inSisallot ||
                    _.every($scope.filterOsaamiset, function(v) {
                        return v;
                    });
        ...
        $scope.processOppiaine = function(oppiaine, vlkIds, inSisallot) {
            ...
            if (_.isEmpty(vlkIds)) {
                vlkIds = _(oppiaine.vuosiluokkakokonaisuudet)
                    .sortBy(item =>
                        _.first($scope.vuosiluokkakokonaisuudetMap[item._vuosiluokkaKokonaisuus].vuosiluokat)
                    )
            ...
            if (inSisallot) {
                $scope.valittuOppiaine.vuosiluokkakokonaisuudet = _.zipBy(
                    _.filter(oppiaine.vuosiluokkakokonaisuudet, function(vlk: any) {
                ...
                $scope.valittuOppiaine.vuosiluokkakokonaisuudet = _.zipBy(
                    oppiaine.vuosiluokkakokonaisuudet,
                    "_vuosiluokkaKokonaisuus"
                );
            }
        ...
        $scope.$on("navifilters:set", function(event, value) {
            ...
            if (value.vlk) {
                ...
                epMenuBuilder.rakennaSisallotOppiaineet(
                    oppiaineet,
                    $scope.navi.sections[2].model.sections,
                    selectedFilters(0)
                );
                epPerusopetusStateService.setState($scope.navi);
            }
        ...
        function selectedFilters(sectionId) {
            return _($scope.navi.sections[2].model.sections[sectionId].items)
                .filter("$selected")
                .map("value")
                .value();
        }
        ...
        $scope.navi = {
            ...
            sections: [
                ...
                },
                {
                    ...
                    id: "vlk",
                    items: epMenuBuilder.rakennaVuosiluokkakokonaisuuksienSisalto(
                    ...
                    model: {
                        sections: [
                            ...
                            },
                            {
                                ...
                                $open: true,
                                items: _.map(
                                    ["tehtava", "tyotavat", "ohjaus", "arviointi", "sisaltoalueet", "tavoitteet"],
                                    function(item, index) {
                                        return {
                                            label: "perusopetus-" + item,
                                            value: item,
                                            depth: 0,
                                            $selected: true,
                                            order: index
    ...
    export const epPerusopetusTekstikappaleController = (
        $scope,
        tekstikappale,
