        ...
        .state("root.selaus.jarjestajat", {
            ...
            views: {
                "": {
                    ...
                    controller: ($scope, $timeout, $q, $state, $stateParams, koulutustoimija, yhteiset, opsHaku, PerusteApi, tiedotteet) => {
                        ...
                        async function haeOpetussuunnitelmista() {
                            ...
                            $timeout(async () => {
                                try {
                                    canceler = $q.defer();
                                    const opsit = await opsHaku.withHttpConfig({
                                    ...
                                    $timeout(() => {
                                        _.forEach($scope.opetussuunnitelmat, async (ops) => {
                                            ...
                                            if (!tutkintonimikeCache[ops.peruste.perusteId]) {
                                                try {
                                                    const tutkintonimikekoodit = await PerusteApi.one("perusteet", ops.peruste.perusteId).all("tutkintonimikekoodit").getList();
                                            ...
                                            $timeout(() => {
                                                $scope.$apply(() => {
                                                    ops.$$osaamisalat = _.isObject(perusteCache[ops.peruste.perusteId])
                                                        ...
                                                        : [];
                                                    ops.$$tutkintonimikkeet = tutkintonimikeCache[ops.peruste.perusteId] || [];
                                                });
                                            });
                                        });
                                    });
